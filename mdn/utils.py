...
def load_custom_data(data_desc, target):
    ...
    data_embed = pd.read_csv('../datasets/'+data_desc+'_embed.csv')
    data_orig = pd.read_csv('../datasets/'+data_desc+'.csv')

    y = np.array(data_sc[target])
    df_sub = data_sc.loc[:, data_sc.columns != target]
    df_orig_sub = data_orig.loc[:, data_orig.columns != target]
    x = np.array(df_sub)
    x_orig = np.array(df_orig_sub)

    features = list(df_sub.columns)

    categorical_columns_selector = selector(dtype_include=object)
    numerical_columns_selector = selector(dtype_exclude=object)
    num_cols = numerical_columns_selector(df_orig_sub)
    cat_cols = categorical_columns_selector(df_orig_sub)
    # get unique values of categorical features
    cat_unique = {}
    for col in cat_cols:
     cat_unique[col] = data_orig[col].unique()

    # get transformer object
    transformer = MinMaxScaler()
    temp = transformer.fit(data_orig[num_cols])


    return x, y, x_orig, features, cat_cols, num_cols, cat_unique, data_sc, data_embed, transformer


'''
get standard deviations for numerical features
'''
def get_numerical_std(data_desc, target_col):
    df = pd.read_csv('../datasets/'+data_desc+'.csv')
    # ignore target column
    dataset_sub = df.loc[:, df.columns != target_col]
    ...
    for col in numerical_columns:
        std_dict[df.columns.get_loc(col)] = df[col].std()
