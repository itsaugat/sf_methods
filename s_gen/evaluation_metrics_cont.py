...
from genetic_helper_cont import *
...
'''
def calculate_lipschitz(x, sf, clf, X_train, continuous_features, categorical_features, action_meta, cat_idxs, actionable_idxs, REACH_KNN, label, num_perturbations, cont_feats_len, LIPS_DIV):
    # get perturbations
    perturbations = generate_perturbations(x, num_perturbations, cont_feats_len)
    perturb_sfs = []
    ...
    for i in range(len(perturbations)):
        ...
        sf_list = sgen_genetic(clf, test_q, X_train, continuous_features, categorical_features, action_meta, cat_idxs,
                               actionable_idxs, REACH_KNN, label, LIPS_DIV)

        if len(sf_list) > 0:
            # get first sf
            sf_p = np.array(sf_list[0])
    ...
    if(len(perturbations) > 0):
        amplification = np.linalg.norm(sf - perturb_sfs, axis=1) / np.linalg.norm(x - perturbations, axis=1)
