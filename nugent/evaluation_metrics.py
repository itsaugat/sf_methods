...
from nugent_helper import get_nugent_sf
...
def calculate_ood(nbrs, y_train, pred, sf):
    ...
    nn = True
    # get distances and indices for neighbors of the computed SF
    distances, indices = nbrs.kneighbors(sf)
...
'''
def calculate_lipschitz(x_orig, x_sf, num_perturbations, nbrs, x_train, y_train, min_num_class, pred):
    # get perturbations
    ...
    for i in range(len(perturbations)):
        ...
        sf = get_nugent_sf(ind, dist, x_train, y_train, min_num_class, pred, q)
        if sf is not None:
