...
def process_causal_adult():
    ...
    adult_actionable_features = ["age", "hours-per-week"]
    ...
    # for hours-per-week
    feature_limits[np.where(X.columns == 'hours-per-week')[0]] = np.array([-1e10, X['hours-per-week'].max()])
    ...
    adult_decreasing_actionable_features = ['hours-per-week']
