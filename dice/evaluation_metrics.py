...
from dice_helper import get_dice_sf
from utils import find_nearest_key
import copy
...
def calculate_sparsity(x1, x2):
    count = sum(x == y for x, y in zip(x1, x2))
    return len(x1) - count
...
'''
def generate_perturbations(X, N, cat_idx):
    epsilon = 0.1
    perturbations = []
    ...
    for i in range(N):
        ...
        for index, value in enumerate(X):
            if index in cat_idx:
                # Keep non-float (string or categorical) values unchanged
                perturbation.append(value)
            else:
                # Generate random noise within the specified radius (epsilon)
                noise = np.random.uniform(low=-epsilon, high=epsilon)

                # Add noise only to numeric values, keep other values unchanged
                perturbation.append(value + noise)
...
'''
def calculate_lipschitz(query_sc, sf_sc, label, d, model, transformer, cat_embed, cols, numerical, cat_idx, features_to_vary, num_perturbations):
    # get perturbations
    perturbations = generate_perturbations(query_sc, num_perturbations, cat_idx)
    perturbations_cp = copy.deepcopy(perturbations)

    # no. of dice sfs for lipschitz
    num = 3

    perturb_sfs = []

    for i in range(len(perturbations_cp)):
        q = perturbations_cp[i]

        # replace categorical values
        for id in cat_idx:
            q[id] = find_nearest_key(cat_embed[id], q[id])
        # create df of q in un-scaled format to get dice sf
        q_df = pd.DataFrame([q], columns=cols)
        q_df[numerical] = transformer.inverse_transform(q_df[numerical])

        # get sf of the perturbation
        _, _, sf_p = get_dice_sf(q_df, label, d, model, num, features_to_vary)
        # get the sf from the list
        sf_p = sf_p[0]
        # remove target (label) from the end
        sf_p = sf_p[:-1]

        # convert sf to scaled form
        for idx in cat_idx:
            sf_p[idx] = float(cat_embed[idx][sf_p[idx]])

        sf_p_df = pd.DataFrame([sf_p], columns=cols)
        sf_p_df[numerical] = transformer.transform(sf_p_df[numerical])
        sf_p_sc = sf_p_df.values.tolist()[0]

        perturb_sfs.append(sf_p_sc)


    amplification = np.linalg.norm(sf_sc - perturb_sfs, axis=1) / np.linalg.norm(query_sc - perturbations, axis=1)
