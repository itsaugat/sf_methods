...
'''
def calculate_lipschitz(x_orig, x_sf, num_perturbations, num_perturb_explanations, explanator):
    # get perturbations
    perturbations = generate_perturbations(x_orig, num_perturbations)
    # Get SF of the perturbed samples
    perturb_sfs = np.zeros_like(perturbations)

    for i, item in enumerate(perturbations):
        perturb_sfs[i] = (explanator.compute_diverse_explanations(item, n_explanations=num_perturb_explanations))[0]

    amplification = np.linalg.norm(x_sf - perturb_sfs, axis=1) / np.linalg.norm(x_orig - perturbations, axis=1)
