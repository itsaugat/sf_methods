...
class CounterfactualExamples:
    ...
    def _get_cfs(self):
        query = self.test_instance_df.values.tolist()[0]
        query_pred = self.test_pred
        cfs = self.final_cfs_df.values.tolist()
        return query, query_pred, cfs

    def visualize_as_list(self, display_sparse_df=True, show_only_changes=False):

        # # custom code to retreive only relevant values
        # query = self.test_instance_df.values.tolist()[0]
        # query_pred = self.test_pred
        # new_outcome = self.new_outcome
        # cfs = self.final_cfs_df.values.tolist()
        # return query, query_pred, new_outcome, cfs
