...
from piece_helper import get_counterfactual
from data_analysis import lending_club_dict
...
'''
def generate_perturbations(X, N, cont_feats):
    X_temp = X.copy()
    epsilon = 0.1
    # Initialize an array to store perturbations
    perturbations = np.zeros((N, len(X)))

    # Generate N perturbations ---> only continuous features
    for i in range(N):
        # Generate random noise within the specified radius (epsilon)
        noise = np.random.uniform(low=-epsilon, high=epsilon)

        # Add noise to the original query's continuous features to create a perturbation
        X_temp[:cont_feats] = X_temp[:cont_feats] + noise

        # Store the perturbation in the array
        perturbations[i, :] = X_temp
...
'''
def calculate_lipschitz(query, query_sc, sf, sf_sc, X_train, cf_df, continuous_feature_names, categorical_feature_names,
                                  clf, action_meta, cat_idxs, df_train, enc, train_preds, num_perturbations, cont_feats_len, cont_feats_idx, cat_feats_idx):
    # get perturbations
    perturbations = generate_perturbations(query, num_perturbations, cont_feats_len)
    perturb_sfs = []

    # get sf of each perturbation
    for i in range(len(perturbations)):
        test_q = perturbations[i]
        # get dictionary of categorical features in label encoding format
        cat_feats_q = test_q[cont_feats_len:]
        ...
        sf_p, _, _ = get_counterfactual(test_q, X_train, cf_df, continuous_feature_names, categorical_feature_names,
                                  clf, action_meta, cat_idxs, test_q_dict, df_train, enc, train_preds)

        # append to perturb_sf list
        perturb_sfs.append(sf_p)

        ######### change categorical encoding of sf_p to normalized encoding (also need to change perturbations encoding)############
        # get sf in the scaled format (continuous + categorical)
        # cont_feats = sf_p[:cont_feats_len]
        # cat_feats = sf_p[cont_feats_len:]
        # cat_feats = enc.inverse_transform(cat_feats.reshape(1, -1))
        # cat_feats = [mapping_dict[val] for val, mapping_dict in zip(cat_feats[0], mapping_dicts)]
        #
        # index_list = sorted(cont_feats_idx + cat_feats_idx)
        # sf_p_sc = [cont_feats[cont_feats_idx.index(i)] if i in cont_feats_idx else cat_feats[cat_feats_idx.index(i)] for i
        #          in index_list]
