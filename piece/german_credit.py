...
from data_analysis import actionability_constraints, german_credit_dict
...
def piece_loop(X_test, X_test_sc, y_test, X_train, X_train_sc, y_train, df_train, df_test, clf, enc, train_preds,
    ...
    for test_idx in range(len(X_test)):
        ...
        # print(cat_feats[0])
        cat_feats = [mapping_dict[val] for val, mapping_dict in zip(cat_feats[0], german_credit_dict)]
        # print(cat_feats)
...
if __name__ == '__main__':
    ...
    TARGET_NAME = 'class'
    cont_feats = ['duration', 'credit_amount', 'installment_commitment', 'residence_since', 'age', 'existing_credits', 'num_dependents']
    cat_feats = ['checking_status', 'credit_history', 'purpose', 'savings_status', 'employment', 'personal_status', 'other_parties', 'property_magnitude', 'other_payment_plans', 'housing', 'job', 'own_telephone', 'foreign_worker']

    cont_feats_idx = [1, 4, 7, 10, 12, 15, 17]
    cat_feats_idx = [0, 2, 3, 5, 6, 8, 9, 11, 13, 14, 16, 18, 19]
