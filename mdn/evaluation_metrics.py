...
from mdn_helper import get_mdn_sf
...
'''
Generate perturbations for MDN-SF
'''
def generate_mdn_perturbations(X, N, cat_cols, cat_unique, features):
    epsilon = 0.1
    perturbations = []

    for feat in features:
        if feat in cat_cols:
            uniques = cat_unique[feat]
            perturbs = np.random.choice(uniques, size=N)
        else:
            val = X[feat]
            perturbs = val + np.random.uniform(-epsilon, epsilon, size=N)

        perturbations.append(perturbs)

    samples = pd.DataFrame(dict(zip(features, perturbations)))
...
'''
def calculate_lipschitz(query, mdn_sf, num_perturbations, query_orig, label, target, features, num_idx, cat_idx, cat_cols, num_cols, cat_unique, cat_embed, df_embed, df_sc, std_dict, transformer):
    # convert query to dictionary
    row = {}
    for a, b in zip(features, query_orig):
        row[a] = b
    # get perturbations
    perturb_dfs = generate_mdn_perturbations(row, num_perturbations, cat_cols, cat_unique, features)

    # normalize the perturbations (samples)
    p_sc = np.array(perturb_dfs)
    ...
    perturb_df_sc = pd.DataFrame(p_sc, columns=features)
    # normalize numerical feature values
    perturb_df_sc[num_cols] = transformer.transform(perturb_df_sc[num_cols])

    perturb_np = [row for row in perturb_dfs.to_numpy()]
    perturb_np_sc = [row for row in perturb_df_sc.to_numpy(dtype=float)]

    # Get MDN of perturbed samples
    perturb_sfs = []

    for i in range(len(perturb_np_sc)):
        q = perturb_np[i]
        sf, _ = get_mdn_sf(q, label, target, features, num_idx, cat_idx, cat_cols, cat_embed, df_embed,
                               df_sc, std_dict)
    ...
    amplification = np.linalg.norm(mdn_sf - perturb_sfs, axis=1) / np.linalg.norm(query - perturb_np_sc, axis=1)
