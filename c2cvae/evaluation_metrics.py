...
from c2c_helper import get_c2c_sf
...
def generate_perturbations(X, N):
    ...
    perturbations = np.zeros((N, X.shape[0]))
...
'''
def calculate_lipschitz(query, sf, label, cat_embed, device, vae, c2c_vae, c2c_latent_dims, num_perturbations):
    # get perturbations
    ...
    for i in range(len(perturbations)):
        q = perturbations[i]
        sf_p = get_c2c_sf(q, label, cat_embed, device, vae, c2c_vae, c2c_latent_dims)
